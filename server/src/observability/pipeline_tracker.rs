...
//!
//! Provides observability for the scanner pipeline including:
//! - Batch job tracking
//! - Rolling window rate metrics
//! - Per-user poll/fetch activity tracking
...
use super::common::{format_elapsed_secs, format_table_with_title, Progress};
...
/// Rolling window size for 24-hour user stats (in seconds)
const ROLLING_24HR_SECS: u64 = 24 * 60 * 60;
...
// ============================================================================
// Per-User Rolling 24hr Email Stats
// ============================================================================

/// A timestamped email count for per-user rolling window
#[derive(Debug, Clone)]
struct UserEmailEvent {
    timestamp: Instant,
    count: u64,
}

/// Tracks emails processed per user over a rolling 24-hour window
#[derive(Debug, Default)]
pub struct UserRolling24hrStats {
    /// Events per user: user_id -> list of timestamped counts
    events: HashMap<i32, VecDeque<UserEmailEvent>>,
}

impl UserRolling24hrStats {
    pub fn new() -> Self {
        Self {
            events: HashMap::new(),
        }
    ...
    /// Record emails processed for a user
    pub fn record(&mut self, user_id: i32, count: u64) {
        let now = Instant::now();
        let event = UserEmailEvent {
            timestamp: now,
            count,
        };
        ...
        self.prune_user(user_id, now);
    }

    /// Get total emails processed for a user in the last 24 hours
    pub fn get_24hr_count(&self, user_id: i32) -> u64 {
        self.events
            .get(&user_id)
            .map(|events| events.iter().map(|e| e.count).sum())
            .unwrap_or(0)
    }

    /// Prune entries older than 24 hours for a specific user
    fn prune_user(&mut self, user_id: i32, now: Instant) {
        if let Some(events) = self.events.get_mut(&user_id) {
            let cutoff = now - std::time::Duration::from_secs(ROLLING_24HR_SECS);
    ...
    /// Prune all users (call periodically for cleanup)
    pub fn prune_all(&mut self) {
        let now = Instant::now();
        let user_ids: Vec<i32> = self.events.keys().copied().collect();
...
pub struct PipelineStats {
    /// Whether the pipeline is in on-demand mode
    pub on_demand: bool,
...
pub struct PipelineTracker {
    ...
    /// Per-user rolling 24hr email stats
    user_24hr_stats: Arc<RwLock<UserRolling24hrStats>>,

    /// Whether the pipeline is in on-demand mode
    on_demand: Arc<RwLock<bool>>,
}
...
impl PipelineTracker {
    pub fn new(queues: Arc<PipelineQueues>) -> Self {
        Self {
            ...
            user_24hr_stats: Arc::new(RwLock::new(UserRolling24hrStats::new())),
            on_demand: Arc::new(RwLock::new(false)),
        }
    }

    /// Set on-demand mode status
    pub fn set_on_demand(&self, on_demand: bool) {
        *self.on_demand.write().unwrap() = on_demand;
    }
    ...
    /// Record emails processed for a specific user (for 24hr rolling stats)
    pub fn record_user_processed(&self, user_id: i32, count: u64) {
        let mut stats = self.user_24hr_stats.write().unwrap();
        stats.record(user_id, count);
    }

    /// Get the 24hr email count for a specific user
    pub fn get_user_24hr_count(&self, user_id: i32) -> u64 {
        self.user_24hr_stats.read().unwrap().get_24hr_count(user_id)
    }
    ...
    pub fn get_stats(&self) -> PipelineStats {
        ...
        PipelineStats {
            on_demand: *self.on_demand.read().unwrap(),
    ...
    /// Log the current pipeline status table at info level
    ...
    pub fn get_status_table(&self) -> String {
        ...
        let user_24hr_stats = self.user_24hr_stats.read().unwrap();
